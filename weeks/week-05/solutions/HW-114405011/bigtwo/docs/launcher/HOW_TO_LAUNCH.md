...
如果你想從 `launchers/` 目錄啟動，則可使用 `QuickStart.bat`。
...
| 檔案 | 位置 | 用途 | 推薦度 |
|------|------|------|--------|
| **開始遊戲.bat** | 專案根目錄 | 最簡單的快速啟動 | ⭐⭐⭐⭐⭐ 推薦 |
| `QuickStart.bat` | `launchers/` | 統一啟動器 | ⭐⭐⭐⭐ |
| `RunGame.vbs` | `launchers/` | 隱形啟動 | ⭐⭐⭐ |
...
**A:**  
1. 確認虛擬環境存在（`.venv` 目錄）
2. 執行：`pip install pygame-ce`
3. 或使用 PowerShell 手動執行 `main.py`
...
- **遊戲目錄**：`d:\2026-python\weeks\week-05\solutions\HW-114405011\bigtwo`
- **字元編碼**：UTF-8（支持中文）
- **啟動方式**：`開始遊戲.bat` 或 `QuickStart.bat`
