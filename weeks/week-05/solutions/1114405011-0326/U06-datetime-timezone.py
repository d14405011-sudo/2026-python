...
naive = datetime.strptime(user_input, "%Y-%m-%d %H:%M:%S")  # 將字串解析為 naive 日期時間（不含時區資訊）
aware = naive.replace(tzinfo=central).astimezone(utc)  # 視為本地時區時間並轉換成 UTC
print(f"存 UTC：{aware}")
print(f"顯示台北：{aware.astimezone(ZoneInfo('Asia/Taipei'))}")  # 再轉回本地時區顯示
