...
#
# 本單元重點：
# 1. classmethod 可定義多種「建構物件」的方式（工廠方法）
# 2. cls 代表「目前的 class」，支援繼承
...
class Point:
    ...
    def from_string(cls, s):
        ...
        x, y = map(int, s.split(','))
        return cls(x, y)  # 用 cls 建立物件，支援繼承
...
# CPE 應用：進位制成本表，示範 classmethod 實用場景
class CostTable:
...
#
# 小技巧：classmethod 很適合用來包裝「各種輸入格式」的解析與物件建立流程
